	...
	$.LinkSelect = function (el, options) {
		...
		function createHtml() {
			var id = self.id
				...
				, cssClass = select.className
				// get the current tab index and make sure to copy it
				, tabindex = $select.attr("tabindex");
			var aHtml = [
				  '<a href="#' + self.id + '" id="' + self.id + '_link" class="' + $.trim(options.classes.link + " " + options.classLink  +" " + cssClass.replace("selectbox", "")) + '"' + (tabindex ? ' tabindex="' + tabindex + '"' : '') + ' data-msg-default="' + $select.attr("data-msg-default") + '" style="width: ' + $select.css("width") + ';">'
					, '<span class="' + options.classes.linkText + '">'
						, text
					, '</span>'
					, '<span class="' + options.classes.linkIcon + '"><span></span></span>'
				, '</a>'
				, '<input type="hidden" style="position: absolute; height:0px; top:-100px;" name="' + name + '" id="' + self.id + '" value="' + value + '" data-msg-default="' + $select.attr("data-msg-default") + '" />'
				, '<div class="' + options.classes.container + ' ' + cssClass + '">'
		...
		function showOptions(callback) {
			...
			if (!bFixedWidth) {
				...
				var aw = (false) ? $a.parent().outerWidth(true) : $a.width();
